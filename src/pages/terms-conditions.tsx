...
export default function TermsConditions() {
  ...
  const seoConfig = useMemo(
    () => ({
      title: ts("termsConditions.pageTitle"),
      description: ts("termsConditions.metaDescription"),
      path: "/terms-conditions",
      keywords: [
        "asyncz terms",
        "asyncz service agreement",
        "asyncz legal"
      ],
    ...
    const fetchTermsContent = async () => {
      try {
        ...
        const response = await getLegalPolicy(
            LegalAgreementType.TERMS_CONDITIONS,
          ...
          <section className="py-16 bg-white dark:bg-gray-900">
            <div className="max-w-4xl mx-auto px-4 pt-16 sm:px-6 lg:px-8">
              <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-8">
                {ts("termsConditions.title")}
              </h1>
              <p className="text-gray-600 dark:text-gray-400 mb-8">
                  {ts("termsConditions.lastUpdated") + policyCreatedAt}
              </p>
